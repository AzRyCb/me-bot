...
`.trim();
			...
			case 'getsw': case 'sw': { if (!store.messages['status@broadcast'].array.length === 0) throw 'Gaada 1 status pun'; let contacts = Object.values(store.contacts); let [who, value] = m.text.split(/[,|-+&]/); value = value?.replace(/\D+/g, '');
...
// Pakai result buat ambil nama kayak di listsw
const result = {};
story.forEach(obj => {
...
let senderName = hisoka.getName(Object.keys(result)[0]);
